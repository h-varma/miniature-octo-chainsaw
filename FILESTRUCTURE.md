...
* Python source files:
  * The Python package source files are located in the `miniature_octo_chainsaw` directory.
  * `tests/test_miniature_octo_chainsaw.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `miniature_octo_chainsaw`.
